...
preReconFile = '/mnt/micdata1/bnp/2019-3/Lin/results/Lin_ML2/scan254/roi0_Ndp64/MLs_L1_p10_g99_pc0_noModelCon_vp3_vi_mm/Niter5000.mat';
%preReconFile = '/mnt/micdata1/bnp/2019-3/Lin/results/Lin_ML/scan281/roi0_Ndp256/MLs_L1_p5_g100_Ndp128_pc10_bg0.2_vp4_vi_mm_badPixels/Niter200.mat';
%base_path = '/mnt/micdata2/velociprobe/2019-2/Jiang/ptychography/bnp_Lin_ML/';
...
% Geometry
%p.   z = 2.4;        %3                                     % Distance from object to detector
p.   z = 2.5062;        %3                                     % Distance from object to detector
%p.   z = 2.44;                                             % Distance from object to detector 
...
p.   energy = 10.7;                                           % Energy (in keV), leave empty to use spec entry mokev
...
% Data preparation
p.   detector.name = 'Pilatus300K_APS';                           % see +detectors/ folder 
...
p.   scan.type = 'custom';                                  % For APS data: 'custrom' or 'pre_recon' - loads from a ML reconstruction file
...
p.   io.data_descriptor = 'BNP 19c3';                     %added by YJ. A short string that describe data when sending notifications 
...
p.   initial_probe_file = preReconFile;
...
%eng. probe_geometry_model = {'scale'};  % list of free parameters in the geometry model, choose from: {'scale', 'asymmetry', 'rotation', 'shear'}
...
%eng. probe_position_error_max = 200e-9; % maximal expected random position errors, probe prositions are confined in a circle with radius defined by probe_position_error_max and with center defined by original positions scaled by probe_geometry_model
...
eng. update_pos_weight_every = 200; % added by YJ. Allow position weight to be updated multiple times. default = inf: only update once.
...
% added by YJ
eng.fourier_error_threshold = inf;     %moniter reconstruction convergence. stop reconstruction if fourier error is larger than the previous one by given (relative) threshold. Default = inf: no convergence check.
...
%resultDir = '/mnt/micdata2/velociprobe/2019-2/RAVEN_Pillar/results/Pillar_ML_recon/fly209/roi_y_lb45_Ny70_Ndp128_edgeCutoff0.25um/MLs_L1_p10_g50_pc_Niter1_geo_model_rotation_scale_shear_vp5_vi_mm_Niter2000/';

[eng.fout, p.suffix] =  generateResultDir(eng, resultDir, '_d2.5062');
%[eng.fout, p.suffix] =  generateResultDir(eng, resultDir);

fout_old = eng.fout;
%eng.extraPrintInfo = strcat('IC_pillar_fly',num2str(scanNo_s(i)),'_scalingX',num2str(scaling_x));
eng.extraPrintInfo = strcat('BNP_19c3_tomo_scan',num2str(scanNo));
...
eng.extraPrintInfo = strcat('IC_19c1_tomo_fly',num2str(scanNo_s(i)),'(',num2str(i),'/',num2str(length(scanNo_s)),')');
fout_old = eng.fout;
...
% basic recontruction parameters 
% PIE / ML methods                    % See for more details: Odstrčil M, et al., Optics express. 2018 Feb 5;26(3):3108-23.
eng. beta_object = 0.1;                 % object step size, larger == faster convergence, smaller == more robust, should not exceed 1
