...
load('/home/beams2/YJIANG/research/slice/velo-2019-c2/LCO/angles.mat')
%scanNo_s_all = [38:208];
%angles_all = [-85:85];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Niter = 1500;
%
%scanNo_s = [727,707,687,667]; gpu_id = 1; %

%scanNo_s = [751,755,759,763]; gpu_id = 2;
%scanNo_s = [751,755,759,763]-20; gpu_id = 3;
%scanNo_s = [751,755,759,763]-40; gpu_id = 5;
%scanNo_s = [862,866,870,874]; gpu_id = 6;
%scanNo_s = [878,882,886,890]; gpu_id = 7;
%scanNo_s = [894,898,902,906]; gpu_id = 8;
scanNo_s = [691,695]; gpu_id = 9;
%scanNo_s = [910,914,918,922]; gpu_id = 10;
...
roi_label = strcat('1_Ndp',num2str(Ndp));

scan_string_format = 'fly%03d';
Niter_save_results = 500;
...
%base_path = '/mnt/micdata2/velociprobe/2019-2/comm/results/LCO2/';
base_path = '/mnt/micdata2/velociprobe/2019-2/Jiang/ptychography/LCO/';

%recon_dir = '/roi0_Ndp256/MLs_L1_p15_g100_pc0_noModelCon_vp7_vi_mm/MLc_poisson_p15_g100_pc0_noModelCon_vp7_vi_mm/Niter100.mat';
recon_dir = '/roi1_Ndp128/MLs_L1_p10_g200_Ndp64_pc100_updPosWeight100_vp7_vi_Nlayers5_avg_dz8e-06_reg0.5_angle_corr0.5/Niter1500.mat';
%preReconPosFile = '/mnt/micdata2/velociprobe/2019-2/Jiang/ptychography/LCO/fly707/roi1_Ndp128/MLs_L1_p10_g200_Ndp64_pc100_updPosWeight100_vp7_vi_Nlayers5_avg_dz8e-06_reg0.5_angle_corr0.5/Niter1500.mat';

for i=1:length(scanNo_s)
    scanNo = scanNo_s(i);
    proj_ang = angles_all(scanNo_s_all==scanNo_s(i));
    scalingFactor_x = 1./cosd(proj_ang)./(1./cosd(proj_ang+angle_correction));
    affine_matrix_init = [1, 0; 0 ,scalingFactor_x];

    preReconPosFile = strcat(base_path,'fly',num2str(scanNo-4),recon_dir);
    clear p
    %% General
    p = struct();
    eng = struct();
    ...
    % Geometry
    p.   z = 1.92;                                             % Distance from object to detector 
    p.   asize = [Ndp Ndp];                                     % Diffr. patt. array size
    p.   ctr = [floor(Ndp/2)+1, floor(Ndp/2)+1];                % Diffr. patt. center coordinates (y,x) (empty means middle of the array); e.g. [100 207;100+20 207+10];
    ...
    p.   energy = 9.3;                                           % Energy (in keV), leave empty to use spec entry mokev
    ...
    p.   affine_matrix = affine_matrix_init; % Applies affine transformation (e.g. rotation, stretching) to the positions (ignore by = []). Convention [yn;xn] = M*[y;x]. For flOMNI we found in June 2019: = [1 , 0.0003583 ; 5.811e-05 , 1 ]; for OMNY we found in October 2018: = [1 0;tan(0.4*pi/180) 1]; laMNI in June 2018  [1,0.0154;-0.0017,1.01]; laMNI in August [1.01 0.0031; -0.0018 1.00] 
    ...
    p.   queue.path=['/mnt/micdata2/velociprobe/2019-2/RAVEN_IC/results/IC/queue/'];      % Folder where the queue of files is defined, note the content of files can overwrite some parameters in p-structure
    ...
    p.   positions_file = ['/mnt/micdata2/velociprobe/2019-2/RAVEN_IC/results/IC/fly145/data_roi0_para.hdf5'];    %Filename pattern for position files, Example: ['../../specES1/scan_positions/scan_%05d.dat']; (the scan number will be automatically filled in)
    ...
    p.   io.data_descriptor = 'VP 19c1 LCO';                     %added by YJ. A short string that describe data when sending notifications 
    ...
    p.   io.SMS_sleep = 1;                                   % max 1 message per SMS_sleep seconds
    ...
    %p.   initial_probe_file = '/mnt/micdata2/velociprobe/2019-2/Deng_3D/results/Hua/ML_recon/fly1004/roi1_Ndp64/dx5.6356e-08_probe0_fly1001/MLs_amplitude_p5_g135_LSQdamping0.1_pc_Niter0_vp_vi/Niter50.mat';
    ...
    p.   ortho_init_probes = false;                              % Added by YJ. Force orthogonalization of initial probes
    ...
    eng. asize_presolve = [N_dp_recon N_dp_recon];      % crop data to "asize_presolve" size to get low resolution estimate that can be used in the next engine as a good initial guess 
    ...
    eng. probe_position_search = probe_position_search;      % iteration number from which the engine will reconstruct probe positions, from iteration == probe_position_search, assume they have to match geometry model with error less than probe_position_error_max
    ...
    %eng. probe_geometry_model = {'scale', 'rotation', 'shear'};  % list of free parameters in the geometry model, choose from: {'scale', 'asymmetry', 'rotation', 'shear'}
    ...
    % other extensions 
    %eng. background = 0.1;               % average background scattering level, for OMNI values around 0.3 for 100ms, for flOMNI <0.1 per 100ms exposure, see for more details: Odstrcil, M., et al., Optics letters 40.23 (2015): 5574-5577.
    ...
    %resultDir = strcat(p.base_path,sprintf(p.scan.format, scanNo),'/roi',p.scan.roi_label,'_probe0_scan36_p7_Niter1000');
    %resultDir = strcat(p.base_path,sprintf(p.scan.format, scanNo),'/roi',p.scan.roi_label);
    resultDir = strcat('/mnt/micdata2/velociprobe/2019-2/Jiang/ptychography/LCO/',sprintf(p.scan.format, scanNo),'/roi',p.scan.roi_label);
    
    [eng.fout, p.suffix] =  generateResultDir(eng, resultDir, strcat('_angle_corr',num2str(angle_correction)));
    %[eng.fout, p.suffix] =  generateResultDir(eng, resultDir);
    fout_old = eng.fout;
    eng.extraPrintInfo = strcat('IC_19c2_LCO_fly',num2str(scanNo_s(i)),'(',num2str(i),'/',num2str(length(scanNo_s)),')');
    ...
    if N_refine > 0
        ...
        % general 
        eng. number_iterations = N_refine;          % number of iterations for selected method 
        eng. asize_presolve = [N_dp_recon_r N_dp_recon_r];      % crop data to "asize_presolve" size to get low resolution estimate that can be used in the next engine as a good initial guess 
        %eng. share_probe = 1;                 % Share probe between scans. Can be either a number/boolean or a list of numbers, specifying the probe index; e.g. [1 2 2] to share the probes between the second and third scan. 
        %eng. share_object = 0;                % Share object between scans. Can be either a number/boolean or a list of numbers, specifying the object index; e.g. [1 2 2] to share the objects between the second and third scan. 
        eng. align_shared_objects = false;     % before merging multiple unshared objects into one shared, the object will be aligned and the probes shifted by the same distance -> use for alignement and shared reconstruction of drifting scans  
