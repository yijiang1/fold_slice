...
dk = 0.033064825413611; 
variable_probe = true;
variable_probe_modes = 1;
base_path = '/home/beams0/YJIANG/ptychography/electron/mos2/';
Ndpx=128;  % size of diffraction
%preReconPosFile = '/home/beams0/YJIANG/ptychography/electron/twisted_mos2/scan001/roi0_Ndp256/MLs_L1_p9_g64_Ndp128_pc10_noModelCon_vp3_vi_mm_affine/Niter500.mat';
%preReconPosFile = '/home/beams0/YJIANG/ptychography/electron/twisted_mos2/scan001/roi0_Ndp256/MLs_L1_p9_g64_Ndp64_pc10_noModelCon_vp3_vi_mm_affine/Niter500.mat';
rot_ang_s = [0];

for i=1:length(rot_ang_s)
    rot_ang = rot_ang_s(i);
    %% General
    clear p 
    clear eng
    ...
    p.   scan_number = [22];                                    % Multiple scan numbers for shared scans
    ...
    %mono mos2 p.   energy = 1.24e-9*0.032029999530735;                                           % Energy (in keV), leave empty to use spec entry mokev

    p.   energy = 80;                                           % Energy (in keV), leave empty to use spec entry mokev
    ...
    p.   affine_matrix = affine_mat ; % Applies affine transformation (e.g. rotation, stretching) to the positions (ignore by = []). Convention [yn;xn] = M*[y;x]. For flOMNI we found in June 2019: = [1 , 0.0003583 ; 5.811e-05 , 1 ]; for OMNY we found in October 2018: = [1 0;tan(0.4*pi/180) 1]; laMNI in June 2018  [1,0.0154;-0.0017,1.01]; laMNI in August [1.01 0.0031; -0.0018 1.00] 
    ...
    p.   scan.nx = 51;        %size(dp,3)                                  % raster scan: number of steps in x
    p.   scan.ny = 87;                                          % raster scan: number of steps in y
    p.   scan.step_size_x = 0.19;                               % raster scan: step size (grid spacing)
    p.   scan.step_size_y = 0.19;                               % raster scan: step size (grid spacing)
    ...
    p.   io.data_descriptor = 'mono mos2';                     %added by YJ. A short string that describe data when sending notifications 
    ...
    %p.   initial_probe_file = '/home/beams0/YJIANG/ptychography/electron/mos2/22/roi6/MLs_L1_p3_g51_pc10_noModelCon_vi_mm_rot_ang0/Niter50.mat';
    ...
    eng.extraPrintInfo = strcat('mono_mos2_scan',num2str(p.   scan_number));
    fout_old = eng.fout;
    ...
    % general 
    eng. number_iterations = 100;          % number of iterations for selected method 
    % Share object between scans. Can be either a number/boolean or a list of numbers, specifying the object index; e.g. [1 2 2] to share the objects between the second and third scan. 
    eng. align_shared_objects = false;     % before merging multiple unshared objects into one shared, the object will be aligned and the probes shifted by the same distance -> use for alignement and shared reconstruction of drifting scans  
    ...
    % add engine
    [eng.fout, p.suffix] =  generateResultDir(eng, fout_old);
    eng.extraPrintInfo = strcat('Twisted_mos2_scan',num2str(p.   scan_number));
    fout_old = eng.fout;
